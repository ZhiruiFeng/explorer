...
def adjust_length_l1(raw_length, embedding, the_node, r):
    nodes = list(embedding.keys())
    length = {}
    for node in nodes:
        length[node] = raw_length[node]
        i = 0
        while i < r:
            length[node] -= abs(embedding[node][i]-embedding[the_node][i])
            if (length[node])<0.00001:
                length[node] = 0
            i = i+1
    return length

def fastmap_L1_store_noupgrade(G, K, epsilon, dis_store, PNumber):
    NG = G.copy()
    embedding = {}
    # initial the embedding as a dict
    for node in list(NG.nodes()):
    ...
    for r in range(K):
        ...
        for t in range(C):
            raw_length = nx.single_source_dijkstra_path_length(NG, node_a)
            length = adjust_length_l1(raw_length, embedding, node_a, r)
            node_c = max(length.items(), key=operator.itemgetter(1))[0]
            if node_c == node_b:
                break
            else:
                ...
                node_a = node_c
        raw_length_a = nx.single_source_dijkstra_path_length(NG, node_a)
        length_a = adjust_length_l1(raw_length_a, embedding, node_a, r)
        raw_length_b = nx.single_source_dijkstra_path_length(NG, node_b)
        length_b = adjust_length_l1(raw_length_b, embedding, node_b, r)
        if r < PNumber:
            store_undirected_distances(G, dis_store, node_a, nx.single_source_dijkstra_path_length(G, node_a))
            store_undirected_distances(G, dis_store, node_b, nx.single_source_dijkstra_path_length(G, node_b))
        dis_ab = length_a[node_b]
